...
export class LoginComponent implements OnInit {
  ...
  addUser(userForm: NgForm) {
    ...
    this.authService
      .createUserWithEmailAndPassword(
        userForm.value["emailIdReg"],
        userForm.value["passwordReg"]
      )
      .then((res) => {
        const user = {
  ...
  signInWithEmail(loginForm: NgForm) {
    this.authService
      .signInRegular(loginForm.value["emailId"], loginForm.value["loginPassword"])
      .then((res) => {
        this.toastService.success(
          "Authentication Success",
          "Logging in please wait"
        );
        localStorage.setItem('currentUserShop',JSON.stringify({token:'test', name:this.loginUser.emailId, password:this.loginUser.password}))
